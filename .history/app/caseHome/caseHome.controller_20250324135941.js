...
export const getCaseHomes = asyncHandler(async (req, res) => {
  try {
    ...
    const caseHomes = await prisma.caseHome.findMany({
      ...
      include: {
        developer: true,  // Используем связь через developerId
        categories: true, // Привязываем категории через связь
      },
    });
...
export const getCaseHome = asyncHandler(async (req, res) => {
  try {
    const caseHome = await prisma.caseHome.findUnique({
      ...
      include: {
        developer: true,  // Исправлено, чтобы было правильно связано через developerId
  ...
  try {
    ...
    const caseHome = await prisma.caseHome.create({
      data: {
        ...
        developer: { connect: { id: developerId } },  // Связываем через developerId
        categories: {
