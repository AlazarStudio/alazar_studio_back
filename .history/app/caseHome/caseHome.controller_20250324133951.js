...
const caseHomes = await prisma.caseHome.findMany({
  ...
  include: {
    developer: true,
    categories: true, // Здесь убедитесь, что категории действительно извлекаются
  },
});
...
export const updateCaseHome = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: 'Failed to update case home', message: error.message });
  }
});
